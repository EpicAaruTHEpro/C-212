...
def handleSendFile(client_name, file_name, file_size):
    ...
    other_client_name = clients[client_name]["connected_with"]
    other_client_socket = clients[other_client_name]["client"]
    msg = f"\n {client_name} wants to send file {file_name} with size {file_size} bytes. Do you want to download? (Y/N)"
    other_client_socket.send(msg.encode())
    time.sleep(1)
    messageToSend = f"Download:{file_name}"
    other_client_socket.send(messageToSend.encode())
