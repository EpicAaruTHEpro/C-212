    ...
    while True:
        ...
        try:
            if("tiul" in chunk.decode() and "1.0," not in chunk.decode()):
                ...
                textarea.insert(END, "\n"+chunk.decode('ascii'))
                textarea.see("end")
            elif("decline your request..." in chunk.decode()):
                labelchat.configure(text="")
                textarea.insert(END, "\n"+chunk.decode('ascii'))
                textarea.see("end")
                ...
                getFileName = chunk.decode('ascii').split(":")
...
def browseFiles():
    ...
    try:
        ...
        if message[:4] == "send":
            ...
            textarea.insert(END, "file sent successfully")
...
def sendMessage():
   ...
   SERVER.send(messageToSend.encode("utf-8"))
   textarea.insert(END,"\n You > " + messageToSend)
   textarea.see("end")
   text_message.delete(0, "end")

   if(messageToSend == "y" or messageToSend == "Y"):
